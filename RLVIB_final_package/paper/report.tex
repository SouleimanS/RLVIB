...
%
% Uses the official Ecole Polytechnique document class (polytechnique.sty, gitlab.binets.fr/
% typographix/polytechnique), copied in alongside this file. It requires five brand-asset PDFs in
% figures/ (polytechnique-{logovert,logohori,filetlongrouge,filetcourt,armes}.pdf), all real,
% official files -- see figures/README.md.
%
% Dependencies beyond polytechnique.sty itself (which pulls in calc, xcolor, geometry, graphicx,
% ifthen, ifpdf, lmodern, titlesec): amsmath, amssymb, booktabs, tikz (positioning, arrows.meta),
% microtype (guarded), hyperref (guarded). No pgfplots (absent on the target machines; previously
% broke the build). No babel -- the report is in English and compiled fine without it before.
...
% polytechnique.sty's \titlepagelayout reserves a fixed 0.475\textheight across three \vspace*
% gaps, sized for the sample document's one-line title/subtitle/author. Our longer title, two-line
% subtitle, and five-line author block push the bottom logo line onto a spurious second page under
% the default gaps; shrinking them (the \fill glue after the author block still pins the logo to
% the page bottom) keeps everything on the title page without touching the vendored .sty.
...
\title[Making Frozen Audio--Visual LLMs Listen]{Making Frozen Audio--Visual LLMs Listen}
\subtitle{A Variational Latent-Variable Model for Audio Grounding, Fit by Regularized
  Preference-Based Estimation}
\author{\itshape Internship Report --- Applied Mathematics: Statistics and Modelling\\[8pt]
  \upshape Souleiman Sbai\\[4pt]
  \'Ecole Polytechnique\\
...
\emph{without touching the backbone}. We formulate a discriminative latent-variable model -- a
variational information bottleneck (VIB) -- attached to each modality stream of a frozen
audio--visual LLM ($<$0.5\% of backbone parameters, zero-initialized so that it is exactly the
identity map at attachment), and estimate its parameters by regularized maximum likelihood under an
\emph{anchored} preference objective on constructed audio--visual conflicts. The unregularized
estimator -- standard direct preference optimization (DPO), itself a Bradley-Terry paired-comparison
likelihood -- collapses to a constant response on these pairs (perception accuracy
$0.95\!\to\!0.10$); two explicit penalty terms -- a chosen-answer anchor and a KL tether to the
frozen backbone on ordinary inputs -- remove this degenerate optimum. On AVHBench ($n=5302$) the
fitted model improves overall accuracy by 5.1
...
pathway was added post hoc). This report documents the statistical model, its estimation procedure,
the evaluation protocol designed to test it honestly, the empirical results with their paired
significance analysis, the mechanistic diagnostics that explain them, and the limitations that bound
the claim.
\end{abstract}
...
four backbones and situates it relative to prior methods. Section~\ref{sec:discussion} discusses
limitations and reproducibility, and Section~\ref{sec:conclusion} concludes.
...
weight matrix directly; Table~\ref{tab:main-results} reports a direct empirical comparison against
LoRA at matched attachment points.
...
\subsection{A Statistical Modeling Perspective}
\label{sec:stat-model}

It is worth stating plainly what Sections~\ref{sec:ib-bounds}--\ref{sec:ib-reparam} amount to as a
statistical model, since the architecture of Section~\ref{sec:method} is easiest to read as an
instance of it. The pair $q(z\mid x)=\mathcal N(\mu(x),\sigma(x)^2I)$ and $q(y\mid z)$ define a
\textbf{discriminative latent-variable model}: $Z$ is a per-token latent with a data-dependent
Gaussian law, and $Y$ (the answer) is generated conditionally on $Z$ rather than on $X$ directly. The
encoder $(\mu(\cdot),\sigma(\cdot))$ plays the role of an \textbf{amortized inference network} in the
sense of Kingma and Welling (2014): rather than fitting a separate variational posterior for every
$x$, a single set of weights maps any $x$ to the parameters of its approximate posterior, which is
what makes the model applicable token-by-token at inference time instead of requiring a fresh
optimization per input. The variational family is restricted to \textbf{diagonal (mean-field)
Gaussians}, a modeling choice with two direct consequences worth stating explicitly rather than
leaving implicit: the posterior over $Z$ cannot represent correlation between latent coordinates, and
it is unimodal, so any genuine multimodality in what $Z$ needs to encode (for instance, two
qualitatively different but equally plausible completions of an ambiguous input) is necessarily
averaged over rather than represented. Fitting this model reduces, by
Equation~\eqref{eq:rvib}, to maximizing a single scalar criterion that is a sum of a
data-log-likelihood term and a KL-divergence penalty -- i.e., \textbf{penalized maximum likelihood}
rather than an unconstrained likelihood maximization. Section~\ref{sec:objective} extends this same
penalized-likelihood view to the full training criterion, adding two further penalty terms to the
likelihood contributed by the preference data.
...
Structurally, Equation~\eqref{eq:dpo} is the negative log-likelihood of a \textbf{Bradley-Terry
paired-comparison model}: writing $s(y)=\beta\big(\log q_\theta(y\mid z)-\log q_{\text{base}}(y\mid
z)\big)$ for the implicit score assigned to a candidate answer $y$, the loss is exactly
$-\log\sigma\big(s(\text{chosen})-s(\text{rejected})\big)$, the standard likelihood for observing
``chosen beats rejected'' when each candidate has latent strength $s(\cdot)$. Minimizing
Equation~\eqref{eq:dpo} over the constructed pairs is therefore ordinary maximum-likelihood
estimation of a Bradley-Terry model in which the adapter parameters $\theta$ enter only through
their effect on the two candidates' scores -- which is exactly what makes the failure mode below an
estimation pathology rather than an implementation bug: an unregularized likelihood fit constrains
only the \emph{difference} between the two scores, leaving their common level free.
...
\label{eq:anchored}
\end{align}
In the language of Section~\ref{sec:stat-model}, Equation~\eqref{eq:anchored} is a
\textbf{penalized-likelihood estimator}: the engine term is the Bradley-Terry log-likelihood of
Section~\ref{sec:dpo-fails}, and the remaining three terms are penalties added to remove specific
pathologies of its unconstrained optimum, each with a standard statistical reading. \textbf{Rail 1}
is an mDPO-style anchor (Wang et al., 2024) and reads as an exterior-penalty relaxation of the
inequality constraint chosen-score $\ge$ base-score: it directly penalizes any drop in the chosen
response's probability relative to the frozen backbone, which is exactly what would happen if
training pushed both candidate probabilities down together. \textbf{Rail 2} penalizes divergence
from the frozen backbone's output distribution on ordinary, non-substituted inputs; it is a
\textbf{proximal (trust-region) penalty} in the same sense used to stabilize policy-gradient methods
such as TRPO and PPO, keeping $p_\theta$ within a KL-divergence neighborhood of $p_{\text{base}}$
rather than restricting it directly in parameter space, which is exactly what preserves capability on
inputs the training data never touches. The uniform-``no'' solution of
...
what the current question is asking about. The query-conditioned variant, illustrated in
...
Four benchmarks are used in this work, each measuring a different aspect of the method's behaviour.
...
without accounting for the sensitivity documented above.
...
$+$ cross-attention (step 60)  & \textbf{0.850} & 0.805 & 0.696 & 0.776 & \multicolumn{3}{c}{---$^\dagger$} \\
$+$ LoRA $r{=}16$ (step 90)    & 0.828 & 0.835 & 0.709 & 0.789 & 0.896 & 0.724 & 0.810 \\
$+$ VIB-DPO (step 60)         & 0.837 & \textbf{0.838} & 0.766 & 0.812 & 0.890 & 0.743 & 0.816 \\
...
Section~\ref{sec:protocol}; adapters selected on a validation half. $^\dagger$CMM evaluation for the
cross-attention baseline had not completed at the time of writing.}
...
unconditional VIB-DPO variant and 5.8 points for the query-conditioned FiLM variant -- while
...
\midrule
Base $\to$ VIB-DPO & 152 / 411 & $+5.2$ [+4.3, +6.1] & Real ($p<10^{-4}$) \\
...
Because the frozen baseline and every trained variant are scored on the exact same items, their
outcomes are \textbf{paired}, not independent: an unpaired test for the difference of two accuracies
(e.g., a two-sample $z$-test on the two overall accuracies) would discard the item-level correlation
between the two outcome sequences and is the wrong test for this design. The appropriate paired test
for binary outcomes conditions on the items where the two models agree and asks only whether the
items where they disagree split evenly between the two directions of disagreement -- this is exactly
McNemar's test. Of $N$ items, write $b$ for those answered correctly by the baseline alone and $c$
for those answered correctly by the adapted model alone, giving
\[
\Delta = \frac{c-b}{N}, \qquad
\text{95\% CI: } \Delta \pm 1.96\,\frac{\sqrt{(b+c)-(c-b)^2/N}}{N}, \qquad
\text{McNemar's test: is } b/c \text{ a fair coin?}
\]
The interval above is the standard normal (Wald) approximation to the sampling distribution of
$\Delta$ under McNemar's test, valid once the discordant count $b+c$ is not too small; every row of
Table~\ref{tab:significance} has $b+c$ in the hundreds, comfortably inside the regime where the
approximation holds. Because per-item predictions are stored for every evaluation run
(Section~\ref{sec:reproducibility}), an exact McNemar test or a nonparametric item-level bootstrap
could be recomputed from the same stored predictions without re-running any model; the Wald interval
is reported here for interpretability, not because a more exact alternative is unavailable.

Table~\ref{tab:significance} reports this analysis on the held-out evaluation split. The discordant
counts for both proposed adapters (411 versus 152 for VIB-DPO; 532 versus 253 for FiLM) are
...
One further caveat applies to every significance claim in this report, not only
Table~\ref{tab:significance}: Tables~\ref{tab:main-results}, \ref{tab:significance},
\ref{tab:mmau}, and \ref{tab:videomme} together report a substantial number of pairwise comparisons across
benchmarks, metrics, and backbones, and no family-wise error-rate correction (e.g.\ Bonferroni or
Benjamini--Hochberg) is applied across them. Each reported $p$-value and confidence interval should
therefore be read as a per-comparison statement rather than as controlling a joint error rate over
the full set of comparisons in this report. The $p<10^{-4}$ results above are conservative enough to
survive any standard correction applied to the handful of primary comparisons they belong to, but the
smaller, non-significant differences discussed in Section~\ref{sec:capability} should likewise not be
over-interpreted as a multiplicity-adjusted null result.
...
adapters incur a small cost: $-1.2$ points for VIB-DPO, $-1.7$ for FiLM. Both differences lie within
...
between the two adapters matches MMAU -- FiLM preserves capability better than VIB-DPO on both
...
answer, that falls on audio and video tokens (Figure~\ref{fig:attn}). Writing $\alpha_i$ for the
eager-attention weight from the answer-generating position to input position $i$ and $\mathcal A$ for
the set of positions whose token carries the \texttt{<|audio\_pad|>} or \texttt{<|video\_pad|>}
prefix -- i.e., audio/video tokens are identified by token type in the sequence, not by position --
the reported quantity is
\[
\text{AV-share} = \sum_{i\,\in\,\mathcal A} \alpha_i .
\]
Because $\{\alpha_i\}$ sums to one over the full input by construction (a softmax row), this sum is
directly the audio/visual tokens' share of the answer position's total attention, computed
independently for each clip; Figure~\ref{fig:attn}'s boxplots report the distribution of this
per-clip share over the evaluated clips. For Qwen3-Omni, this share is
essentially unchanged between the frozen base and the VIB-DPO variant (5.4\% to 5.7\%, within the
variation already present across clips), suggesting that VIB-DPO's accuracy gain comes from
...
Qwen2.5-Omni shows the same pattern as Qwen3-Omni under VIB-DPO, moving only from 7.9\% to 8.1\%.
...
variants are separable along a dimension that accuracy alone cannot reveal: VIB-DPO raises audio
...
\subsection{A Withdrawn Claim: Direct Parameter Inspection}
\label{sec:params}

The third probe requires no inference at all: rather than measuring behaviour on held-out clips, it
reads quantities directly off the trained FiLM checkpoint's parameters. This subsection documents the
one such reading that was actually made public in an earlier stage of the project, and then retracted
-- reported here in full rather than quietly dropped, in keeping with this report's provenance
standard that a withdrawn claim is itself a reportable fact.

\begin{table}[h]
\centering
\small
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{p{5.0cm}p{4.3cm}p{3.3cm}}
\toprule
Claim & Verification method & Outcome \\
\midrule
59\% of the vision stream is rewritten by the adapter with the audio module inactive & Direct
inspection of the trained parameters & Not supported; claim withdrawn \\
\bottomrule
\end{tabular}
\caption{The one parameter-level claim from this project that did not survive verification.}
\label{tab:params}
\end{table}

As originally stated, the claim would have been a strong one: it asserted that a substantial fraction
of the vision pathway is altered by the adapter \emph{regardless of whether the audio module has any
reason, derived from the audio stream, to demand it} -- i.e., that the two modality-specific
bottlenecks are not acting independently even when one of them is inactive. Had it held up, it would
have complicated the reading of every other mechanism result in this report, each of which treats the
audio and vision bottlenecks as separable channels. When the underlying trained parameters were
inspected directly, rather than through whatever derived or indirect quantity had originally produced
the 59\% figure, the claim was not supported, and it was withdrawn before being used in any
downstream analysis. The general point this episode illustrates is a narrow but important one for a
report built on parameter-level diagnostics: a claim computed indirectly is only as trustworthy as its
derivation, and where a direct measurement of the same quantity is available, it should be checked
against it before the claim is reported -- which is exactly the standard the rest of this section's
probes are held to, and precisely how this particular claim was caught.
...
MiniCPM-O 2.6 (8B) & 0.744 & ---$^\dagger$ & --- & --- \\
\bottomrule
\end{tabular}
\caption{AVHBench overall accuracy across four backbones, $n=5302$; identical data, objective,
constraints, and selection procedure applied to every row. $^\dagger$Adapter training on MiniCPM-O
2.6 had not completed at the time of writing.}
\label{tab:backbones}
\end{table}
...
backbone never had in the first place.
...
across publications, for exactly the protocol-sensitivity reason quantified in
Section~\ref{sec:protocol}.}
\label{tab:prior}
\end{table}

Table~\ref{tab:prior} situates this work's result relative to two prior mitigations discussed in
Section~\ref{sec:related}, comparing relative improvements only, since Section~\ref{sec:protocol}
...
where that precondition does not hold (VideoLLaMA2), the method does not help.
Section~\ref{sec:capability} shows a small, consistently negative, though not
...
2.6 (Table~\ref{tab:backbones}) had not completed; a video-substitution training variant was trained
but not yet evaluated; and the CMM evaluation for the cross-attention baseline of
Table~\ref{tab:main-results} had not completed. These are reported as open items rather than
results, consistent with the provenance
...
Reproducibility here is a statistical concern as much as a software one: every number reported in
this document can be traced back to the exact procedure that produced it, because each table and
figure is generated by a single, dedicated script rather than assembled by hand. The protocol choices
of Section~\ref{sec:protocol} -- frame rate, prompts, answer format, and decoding parameters -- are
fixed in code and logged rather than re-tuned between conditions, which is what makes comparisons
across rows of the same table valid in the first place. Checkpoint selection is always performed on a
validation half disjoint from the reported evaluation split, so that no reported accuracy is the
result of selecting on the test set. Finally, per-item predictions are stored for every evaluation
run, which is what makes the paired analysis of Section~\ref{sec:significance} -- and, in principle,
an exact McNemar test or a bootstrap in place of the Wald approximation used there -- recomputable
directly from stored data rather than requiring the models to be re-run.
...
whose audio pathway was integrated post hoc. This supports a general claim about this class of
method: a small, reversible adapter can make existing but under-used information in a frozen model
\emph{usable}, but it cannot supply competence the frozen model never had.

Future work identified during this internship falls into two categories. The most immediate is
completing the experiments already in progress at the time of writing: evaluating the adapter on
MiniCPM-O 2.6, and evaluating the video-substitution training variant that was trained but not yet
assessed (Section~\ref{sec:limitations}). The second is strengthening the statistical claims made
here with a multi-seed variance study and a systematic sweep of the compression coefficient
$\beta_{kl}$, both identified explicitly as gaps in Section~\ref{sec:limitations}.
