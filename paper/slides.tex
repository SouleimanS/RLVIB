...
\begin{tikzpicture}[font=\scriptsize,>={Latex[length=1.5mm]},node distance=2mm and 3.5mm,
  ...
  % bottom row: the NEW question path
  \node[io,below=11mm of enc,xshift=-14mm] (w) {question $w_{1:L}$};
  \node[frzs,right=of w] (embed) {$E$ frozen, mean-pool};
  \node[new,right=of embed] (wq) {$W_q$};
  \node[io,right=of wq] (qv) {$q$};
  \node[new] (wf) at (film.south |- qv) {$W_{\!f}$};
  \node[new] (wg) at (mult.south |- qv) {$w_g,b_g$};
  \draw[->] (w)--(embed); \draw[->] (embed)--(wq); \draw[->] (wq)--(qv);
  \draw[->] (qv)--(wf);
  \draw[->] (qv.south) to[out=-15,in=-165] (wg.south);
  ...
  \node[font=\tiny,below=1mm of embed,xshift=8mm]
...
\vspace{0.5em}
\par
{\small $\gamma$ $=$ a \textbf{scale} on every hidden feature, chosen by the question \quad$\cdot$\quad
$\beta$ $=$ a \textbf{shift} on every hidden feature \quad$\cdot$\quad
$g(q)$ $=$ one \textbf{volume knob} on the whole edit}
\vspace{0.2em}
\[
