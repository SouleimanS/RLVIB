...
\midrule
\textbf{post}-adapter (ours)   & 0.837 & 0.838 & 0.766 & 0.812 & 0.890 & 0.743 \\
...
\item biggest drop: AV-Matching ($0.766\to0.712$) --- integration needs the \emph{fused} token space
\item read: edit \textbf{after} the adapter (LLM space) $>$ edit raw encoder features
\item decision: \textbf{keep post} --- better, one attach dim, same code on every backbone
\end{itemize}
\end{frame}
...
\end{axis}
\end{tikzpicture}
{\scriptsize measured rate at the optimum (mean KL/dim): $0.002$, $0.011$, $0.032$, $0.38$, $1.21$ nats.}
\end{column}
\begin{column}{0.42\textwidth}\small
\textbf{What it shows}
\begin{itemize}\footnotesize
\item plateau $[0.01,\,0.1]$: rate inactive ($\le 0.03$ nats) --- not the mechanism
\item cliff $\ge 0.5$: rate $\times 100$, grounding $\mathbf{-3.6}$ --- over-compression
\item optimum $0.1$ --- verdict: \textbf{guard-rail, not driver}
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\midrule
full VIB (noise $+$ rate)          & ---                          & \textbf{0.812} & \textbf{0.743} \\
...
\vfill
\begin{itemize}\small
\item $-$\,noise: $\mathbf{-1.6}$ $\Rightarrow$ \textbf{reparam noise $=$ the active ingredient} (free exploration)
\item $-$\,rate: $-0.3$ $\Rightarrow$ minor --- matches the $\beta_{kl}$ sweep
\item LoRA: $-3.4$ $\Rightarrow$ the \textbf{residual form} matters, not parameter count
\end{itemize}
\end{frame}
...
\item signal is \textbf{cheap} --- the constraint is the \emph{objective}, not data
\end{itemize}
\vspace{0.3em}
...
\item collapse fix $=$ the audio analog of mDPO's anchor --- \emph{validated} prior art, not a guess
\item AVCD stacks on top at decode time --- next composition experiment
\end{itemize}
\end{frame}

% --- Negative results kept (real content) ---
\begin{frame}{What did \emph{not} work --- and what each failure taught us}
\centering
...
\begin{tabular}{l l l}
\toprule
tried & failure mode & lesson $\to$ action \\
\midrule
na\"ive swap-DPO & collapse to constant ``no'' (PA $0.95\!\to\!0.1$) & likelihood displacement $\to$ \textbf{two anchors} \\
GRPO, easy yes/no items & $\mathrm{adv\_std}{=}0$ on $\sim$2/3 steps: no gradient & unanimous groups $\to$ \textbf{train on swap items} \\
FiLM hard gate $g(q)$ & gate never closed ($b_g$ stuck at init) & routing lands in $\gamma,\beta$ $\to$ \textbf{modulated re-encoding} \\
``vision rewrites 59\%'' story & overturned by the weight audit & \textbf{measure, don't assume} (probes now in repo) \\
\bottomrule
\end{tabular}}
\vfill
\begin{itemize}\small
\item every failure is documented in \texttt{docs/research/} with the diagnosis and the fix
\item two of the four became the paper's \emph{content} (the anchors; the mechanism finding)
\end{itemize}
\end{frame}

% --- Methodology rigor (real content) ---
\begin{frame}{Why the numbers can be trusted}
\begin{itemize}\small
\item \textbf{no selection-on-test}: checkpoints picked on a val half; reported on the disjoint held-out
\item \textbf{pinned harness}: per-model fps, fixed answer suffix, per-model system-prompt convention ---
identical for base and every adapter
\item \textbf{paired statistics}: exact McNemar $+$ bootstrap CIs on the same items; yes-rate collapse detector
\item \textbf{leakage check}: AVE (train) $\cap$ AVHBench/CMM (eval) $=\varnothing$ --- scripted, clean
\item \textbf{reproducible}: every table cell regenerates from \texttt{runs/*.json} $+$ one script;
evals resume from checkpoints
\item \textbf{audited}: an independent 3-agent review of the pipeline caught the fps and
selection-on-test bugs \emph{before} the numbers shipped
\end{itemize}
\vfill
\begin{center}
\setbeamercolor{block body}{bg=blue!6}
\begin{block}{}
\centering\small one command $=$ one table: \ \texttt{python scripts/make\_table.py} \quad$\cdot$\quad
significance: \ \texttt{python scripts/paired\_stats.py}
\end{block}
\end{center}
\end{frame}
