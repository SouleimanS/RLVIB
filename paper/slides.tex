...
% figure slot with a build-safe fallback box
\newcommand{\figslot}[2]{\IfFileExists{figures/#1}%
  {\includegraphics[width=#2]{figures/#1}}%
  {\fbox{\parbox[c][2.6cm][c]{#2}{\centering\tiny\ttfamily figures/#1}}}}
...
% --- 18. FiLM formally (i): the model ---
\begin{frame}{FiLM, formally (i): a question-conditioned bottleneck}
Per modality $m\in\{a,v\}$: tokens $x\in\mathbb R^{T\times d}$, question tokens $w_{1:L}$.
\vspace{-0.2em}
\[
q = W_q\,\underbrace{\tfrac1L\textstyle\sum_{t}E(w_t)}_{\text{frozen embed., mean-pool}}
\qquad h=\mathrm{GELU}(W_e x) \qquad [\gamma;\beta]=W_{\!f}\,q
\]
\vspace{-0.7em}
\[
\tilde h=(1+\mathrm{clamp}(\gamma))\odot h+\beta \qquad
z\sim\mathcal N\!\big(\mu(\tilde h),\operatorname{diag}\sigma^2(\tilde h)\big) \qquad
g(q)=\mathrm{sigm}(w_g^{\!\top}q+b_g)
\]
\vspace{-0.7em}
\[
...
\vspace{-0.6em}
\begin{itemize}\small
\item \textbf{Proposition (identity at init, for every $q$).} $W_{\text{out}}{=}0$ and $W_{\!f}{=}0$
($\gamma{=}\beta{=}0$) $\Rightarrow y\equiv x$ \emph{and} the KL rate equals the unconditional VIB's.
\emph{Proof:} the residual is $g\cdot 0\cdot z=0$, any $q,g$. $\square$\quad
Consequence: \texttt{bypass} still returns the \textbf{exact frozen base} --- the DPO reference stays free.
\item Trainable per modality: $W_e,W_\mu,W_\sigma,W_{\text{out}},W_{\!f},w_g$ ($+$ shared $W_q$); backbone frozen.
\end{itemize}
\end{frame}

% --- 19. FiLM formally (ii): the information view ---
\begin{frame}{FiLM, formally (ii): compress what is irrelevant \emph{to this question}}
\textbf{Unconditional VIB} minimizes a variational bound on the \emph{rate} $I(Z;X)$ ---
the same compression for every question.
\vspace{0.3em}
\begin{center}
...
\end{center}
\vspace{-0.2em}
{\small The \emph{same} KL term, read conditionally: for each question $q$ the standard bound
$I(Z;X\mid Q{=}q)\le\mathbb E_x\,\mathrm{KL}(q_\phi\Vert r)$ holds with the $q$-independent prior $r=\mathcal N(0,I)$;
averaging over $q$ gives the boxed inequality. FiLM makes the posterior $q$-dependent, so the code can
\textbf{keep the bits of the stream that answer \emph{this} question and discard the rest} ---
an audio question keeps audio bits; a visual question does not pay for them.}
\vfill
{\footnotesize Honest note: at $\beta_{kl}{=}0.05$ the rate binds only weakly --- the conditional-rate view is
the \emph{design principle}; the conditioning (FiLM $+$ gate) does the empirical work.}
\end{frame}

% --- 20. FiLM formally (iii): the forcing lemma ---
\begin{frame}{FiLM, formally (iii): why the question \emph{must} be read}
