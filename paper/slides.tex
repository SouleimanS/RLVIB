...
\end{tabular}
\vfill
{\footnotesize Every item is answered by \emph{both} models, so the comparison lives in the
\textbf{disagreements}: out of $N$ paired items, $b$ $=$ items only the \emph{left} model gets right,
$c$ $=$ items only the \emph{right} model gets right. The gain and its uncertainty follow directly:}
\vspace{-0.3em}
\[
\Delta \;=\; \frac{c-b}{N}
\qquad\qquad
...
\]
\vspace{-0.7em}
{\footnotesize\emph{McNemar} then asks whether the $b/c$ split could be a fair coin (exact binomial
under $H_0\!: c/(b{+}c){=}\tfrac12$): $^{*}$ $=$ $p<0.05$ with $b,c$ \textbf{pooled} over the 3
training seeds\ \ $\cdot$\ \ $^{\dagger}$ $=$ marginal ($0.05\le p<0.10$)\ \ $\cdot$\ \ no mark $=$ tie.
$\Delta$ $=$ mean$\pm$std across seeds; held-out $=$ full set minus the 300-item selection subset
(select on val, report here).\\
