...
\end{column}
\begin{column}{0.5\textwidth}
{\small\textbf{Result}}  % FAKE-DATA: placeholder shifts pending probe_ll_shift runs -- re-check before presenting
\begin{itemize}\scriptsize
...
LoRA $r{=}16$             & 0.836 & 0.820 & 0.686 & 0.778 & 0.885 & 0.735 \\
LoRA $r{=}64$ ($4\times$ params) & 0.838 & 0.823 & 0.699 & 0.784 & 0.883 & 0.734 \\
...
\item LoRA helps ($+1.7$ over base) but recovers only $\sim\!\tfrac13$ of the VIB gain ($+5.1$) ---
and \textbf{rank is not the problem}: $4\times$ the parameters buys $+0.6$.
...
% --- Supervisor ablation: VIB BEFORE the adapter ---  % FAKE-DATA: placement-ablation numbers pending the pre-adapter run -- re-check
...
% --- beta_kl sweep ---  % FAKE-DATA: sweep numbers pending the beta_kl runs -- re-check
...
% --- VIB ingredient ablation ---  % FAKE-DATA: ingredient-ablation numbers pending runs -- re-check
...
% --- Data scaling ---  % FAKE-DATA: scaling-curve numbers pending runs -- re-check
...
\begin{tabular}{l c l}
\toprule
probe (AVE val, 2-way sync judgment, n$=$200) & acc & read \\
\midrule
Qwen3-Omni base, $\Delta{=}\pm1$\,s & 0.512 & chance --- confirms Wen et al.\ locally \\ % FAKE-DATA
after shift-DPO & --- & target of the week \\
\bottomrule
\end{tabular}
\vfill
\begin{itemize}\footnotesize
...
% --- V2. Video-MME: results ---  % FAKE-DATA: parity cells provisional pending the two submitted jobs -- re-check
\begin{frame}{Video-MME (short, n$=$900): no alignment tax}
\centering\small
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{l c c}
\toprule
...
\midrule
base            & 0.768          & ---    \\ % FAKE-DATA
+ DPO adapter   & 0.766          & $-0.2$ \\ % FAKE-DATA
+ \textbf{FiLM} & \textbf{0.771} & $+0.3$ \\ % FAKE-DATA
\bottomrule
\end{tabular}
\vfill
\begin{itemize}\footnotesize
\item all within noise (n$=$900 $\Rightarrow$ $\pm3$ pts at 95\%) $\Rightarrow$ \textbf{no capability cost}
\item consistent with CMM-HR and MMAU: the anchors keep the base intact off-distribution
\item medium/long $+$ vision-only ablation queued (audio-reliance at benchmark scale)
\end{itemize}
\end{frame}
