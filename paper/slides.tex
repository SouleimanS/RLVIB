...
% --- 18.i FiLM math (recap): the question-blind machinery ---
\begin{frame}{Before FiLM: the machinery so far is \emph{question-blind} (recap)}
\textbf{The adapter} (unconditional VIB) --- one transform per modality $m\in\{a,v\}$:
\vspace{-0.2em}
\[
h=\mathrm{GELU}(W_e x)\qquad
z\sim\mathcal N\!\big(\mu(h),\operatorname{diag}\sigma^2(h)\big)\qquad
y=x+W_{\text{out}}\,z
\]
\textbf{Its KL term is a \emph{rate}}: the $\mathrm{KL}_{\text{VIB}}$ in the loss,
$\ \mathbb E_x\,\mathrm{KL}\big(q_\phi(z\,|\,x)\,\|\,\mathcal N(0,I)\big)$, is a variational
\textbf{upper bound on $I(Z;X)$} --- how many bits of the stream the code keeps.
\vspace{0.2em}
\textbf{The objective} (from the fix slide):
\vspace{-0.3em}
\[
\mathcal L=\underbrace{-\log\sigma\big(\beta[(c_p{-}c_r)-(r_p{-}r_r)]\big)}_{\text{swap-DPO}}
+\;\lambda_a\underbrace{(\cdots)}_{\text{chosen anchor}}
+\;\lambda_{kl}\,\mathrm{KL}(p_{\text{base}}\|\,p_\theta)
+\;\beta_{kl}\,\mathrm{KL}_{\text{VIB}}
\]
\vfill
\begin{center}
...
\begin{block}{}
\centering\small \textbf{Every symbol is question-blind}: $\mu(x),\sigma(x)$, the code, the edit are
