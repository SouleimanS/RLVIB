...
At $\beta_{kl}{=}0.05$ the rate binds weakly: it is the \emph{design principle}; the conditioning does the empirical work.
\end{minipage}
\end{center}
\end{frame}
...
\vspace{0.15em}
{\scriptsize Here $\sigma(m)=\tfrac{1}{1+e^{-m}}$ is the \emph{logistic sigmoid} (\textbf{not} the code's
std $\sigma(h)$): DPO models the preference probability $P(\text{chosen}\succ\text{rejected})=\sigma(m)$
of the margin $m$ (Bradley--Terry), and \textbf{the log is its negative log-likelihood}: loss $=-\log\sigma(m)$.}
\vspace{0.15em}
\begin{center}
\setbeamercolor{block body}{bg=blue!6}
\begin{block}{Lemma (question-blind adapters have a loss floor)}
\footnotesize For any adapter that ignores $q$, the two margins on $x^\star$ are exact negatives,
...
\end{block}
\end{center}
{\footnotesize\textbf{Example.} Question-blind, $m{=}2$: losses $0.13 + 2.13 = 2.26$.\quad
Question-blind, $m{=}0$: $0.69+0.69=1.39$ (the floor).\quad
\textbf{Question-aware}: $m_{\text{hear}}{=}m_{\text{see}}{=}2 \Rightarrow 0.13+0.13=0.26\ (\to 0)$.}
\vspace{0.2em}
...
\vspace{0.15em}
{\scriptsize The minimizer we actually find (from the weights): conditioning lands in the \emph{vision}
...
\end{tabular}
\vfill
{\footnotesize $^{*}p<0.05$, $^{\dagger}p<0.10$ --- exact McNemar on item-paired predictions $+$ bootstrap CIs.\\
Held-out $=$ full set minus the 300-item selection subset (select on val, report here).\\
Yes-rates $\approx 0.5$ (no collapse); CMM-PA unchanged in all three. One honest cost: FiLM's A$\to$V $-1.7^{*}$ vs base.}
\vfill
\begin{center}
...
travels by \textbf{different routes}.}
\vfill
\begin{center}
\setbeamercolor{block body}{bg=green!8}
\begin{block}{}
...
\item $+$\textbf{FiLM}: shift $=\mathbf{-1.28}$ ($\approx 3\times$) --- the answer \emph{depends on the
...
\midrule
LoRA ($r{=}16$)      & 0.742 & 0.721 & barely above base (0.761 / 0.733) \\  % FAKE-DATA: pending lora@90 full eval -- re-check
VIB (DPO)            & 0.812 & 0.743 & \\
...
\item \textbf{The VIB's stochastic zero-init residual form matters}: LoRA on the same attach points
recovers little of the gain and loses HR --- it is not ``any adapter would do''.
\item Caveat: default LoRA config ($r$, lr) --- a small sweep pending.
\end{itemize}
\end{frame}
...
\item \textbf{3 seeds} on FiLM@best --- mean$\pm$std, pooled McNemar (expect $\pm 0.6$)
