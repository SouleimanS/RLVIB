...
\end{block}
\end{center}
{\footnotesize\textbf{Example.}\ \ blind, $m{=}2$: $0.13+2.13=2.26$\quad$\cdot$\quad
blind, $m{=}0$: $0.69+0.69=1.39$ (floor)\quad$\cdot$\quad
\textbf{aware}, $m_{\text{hear}}{=}m_{\text{see}}{=}2$: $0.13+0.13=0.26\ (\to 0)$}
\vspace{0.2em}
{\small\textbf{Corollary.} Any minimizer of the HEAR$+$SEE objective must condition on $q$:
\textbf{routing is forced by data construction} (gap $=2\log 2$), not by a regularizer.}
\end{frame}
...
\end{tabular}
\vfill
{\footnotesize $^{*}$ $=$ \textbf{statistically significant} ($p<0.05$, exact McNemar on item-paired
predictions)\ \ $\cdot$\ \ $^{\dagger}$ $=$ \textbf{marginal} ($0.05\le p<0.10$)\ \ $\cdot$\ \ no mark $=$ a tie.\\
