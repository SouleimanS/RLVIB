...
\vspace{0.3em}
\par
{\scriptsize\emph{McNemar's test}: both models answer the \emph{same} items, so only the
\textbf{disagreements} carry information --- count $b$ $=$ items only the \emph{left} model gets right,
$c$ $=$ items only the \emph{right} model gets right, and test whether $c$ exceeds $b$ beyond chance
(exact binomial under $H_0\!: c/(b{+}c){=}\tfrac12$). ``Pooled'' $=$ $b,c$ summed over the 3 seeds
before testing.}
\vfill
\begin{center}
