...
% Lines tagged '% REAL:' carry MEASURED numbers (file named in the comment).
% Blocks tagged '% !! FABRICATED' are invented placeholders -- never present them as measured.
...
\vspace{0.3em}
{\small $\sigma(m)=\tfrac{1}{1+e^{-m}}$ $=$ the \emph{logistic sigmoid}: DPO sets
$P(\text{chosen}\succ\text{rejected})=\sigma(\text{margin})$ (Bradley--Terry), so
$-\log\sigma$ is its \textbf{negative log-likelihood}.}
\end{frame}

% --- Why it broke + the fix (merged; naive loss and anchored loss side by side) ---
\begin{frame}{Why it broke --- and the fix: two anchors}
{\small\textbf{The na\"ive loss} (swap-DPO $+$ the adapter's rate):}
\vspace{-0.4em}
...
\vspace{-0.9em}
\begin{center}
{\footnotesize\textbf{Likelihood displacement}: the margin can grow while \emph{both} $c_p$ and $r_p$
\emph{fall} $\Rightarrow$ drift to a constant ``no'' --- and $\mathrm{KL}_{\text{VIB}}$ constrains the
adapter's \emph{rate}, not its \emph{output}.}
\end{center}
\vspace{0.1em}
{\small\textbf{The fix --- add two anchors}:}
\vspace{-0.3em}
\begin{align*}
...
\vspace{-0.5em}
\begin{center}
{\small \textcolor{trn}{\textbf{chosen anchor}} pins the chosen log-prob $\ge$ base $\Rightarrow$
protects \textbf{PA}\quad$\cdot$\quad\textcolor{trn}{\textbf{KL-to-base}} keeps non-swap outputs near
base $\Rightarrow$ protects \textbf{HR}}
\end{center}
\end{frame}

% --- Loss anatomy schema: one engine, two rails, one rate ---
\begin{frame}{How the loss works, mechanically: one engine, two rails, one rate}
\centering
\resizebox{0.98\textwidth}{!}{%
\begin{tikzpicture}[font=\scriptsize,>={Latex[length=1.6mm]},
  inp/.style={draw,rounded corners,fill=gray!10,align=center,minimum height=9mm},
  polb/.style={draw,very thick,rounded corners,fill=trn!22,draw=trn!70!black,align=center,minimum height=8mm},
  refb/.style={draw,rounded corners,fill=frz!12,align=center,minimum height=8mm},
  lb/.style={draw,rounded corners,align=center,minimum height=9mm},
  op/.style={draw,circle,inner sep=2.5pt,fill=gray!15}]
  % --- swap stream (the engine + rail 1) ---
  \node[inp] (swap) at (0,0) {\textbf{swap batch}: audio-swapped clip\\``what do you \textbf{hear}?''\\chosen $=$ heard $B$, rejected $=$ seen $A$};
  \node[polb] (pol1) at (5.0,0.95) {policy pass\\adapter \textbf{on} ($\theta$)};
  \node[refb] (ref1) at (5.0,-0.95) {reference pass\\adapter \textbf{bypassed} $=$ base};
  \node[inp]  (vals) at (9.2,0) {log-probs of the two answers\\policy: $c_p,\,r_p$\quad base: $c_r,\,r_r$};
  \node[lb,fill=green!10] (eng)  at (13.9,0.95) {\textbf{engine} --- swap-DPO\\$-\log\sigma(\beta[(c_p{-}c_r){-}(r_p{-}r_r)])$\\\emph{prefer the \textbf{heard} answer, relative to base}};
  \node[lb,fill=red!8]    (rail) at (13.9,-0.95) {\textbf{rail 1} --- chosen anchor\\$\lambda_a\,({-}\log\sigma(\beta(c_p{-}c_r){-}\delta))$\\\emph{chosen must never sink below base} $\Rightarrow$ \textbf{PA}};
  \draw[->] (swap.east) -- (pol1.west);
  \draw[->] (swap.east) -- (ref1.west);
  \draw[->] (pol1.east) -- (vals.west);
  \draw[->] (ref1.east) -- (vals.west);
  \draw[->] (vals.east) -- (eng.west);
  \draw[->] (vals.east) -- (rail.west);
  % --- anchor stream (rail 2) ---
  \node[inp] (anc) at (0,-2.9) {\textbf{anchor batch}: ordinary clip\\everyday question (\emph{no} swap)};
  \node[refb] (both2) at (5.0,-2.9) {policy $+$ bypassed pass\\(same clip, both ways)};
  \node[lb,fill=blue!8] (leash) at (13.9,-2.9) {\textbf{rail 2} --- KL-to-base\\$\lambda_{kl}\,\mathrm{KL}(p_{\text{base}}\,\Vert\,p_\theta)$\\\emph{change \textbf{nothing} on normal inputs} $\Rightarrow$ \textbf{HR}};
  \draw[->] (anc.east) -- (both2.west);
  \draw[->] (both2.east) -- node[above]{\tiny $p_\theta$ vs $p_{\text{base}}$} (leash.west);
  % --- rate ---
  \node[lb,fill=gray!12] (rate) at (13.9,-4.35) {\textbf{rate} --- $\beta_{kl}\,\mathrm{KL}_{\text{VIB}}$\\\emph{keep the adapter's code $z$ cheap} (from every policy pass)};
  % --- sum + gradient ---
  \node[op] (sum) at (17.6,-1.6) {$\boldsymbol\Sigma$};
  \draw[->] (eng.east)   -| (sum.north);
  \draw[->] (rail.east)  -- (sum.west);
  \draw[->] (leash.east) -- (sum.west);
  \draw[->] (rate.east)  -| (sum.south);
  \draw[dashed,very thick,trn!80!black,->] (sum.south) -- ++(0,-3.6) -| node[below,pos=0.25]{\tiny $\nabla_\theta$ --- the gradient updates \textbf{only the adapter}; the backbone never moves} (pol1.south);
\end{tikzpicture}}
\vspace{0.4em}
\par
{\small\textbf{Why this cannot collapse:} answering ``no'' to everything used to be free --- now it
sinks the chosen answer (\textcolor{red!70!black}{rail 1 fires}) and shifts normal-input behavior
(\textcolor{blue!70!black}{rail 2 fires}). \emph{The only way to lower the
\textcolor{green!50!black}{engine}'s loss is to actually use the audio.}}
\end{frame}
...
\textbf{Read, left to right:} $I(Z;X\,|\,Q)$ $=$ the bits the code $z$ keeps about the stream $x$
\emph{once the question is known} --- and it is capped by the \emph{same} KL penalty we already pay in
the loss (the $q$-independent prior makes the bound hold per question; average over $q$).\\
\textbf{Practical meaning:} the bit-budget is now spent \emph{per question} --- keep audio bits for an
audio question, drop them for a visual one --- instead of one fixed allocation for all questions.
\end{minipage}
\end{center}
\end{frame}
...
{\footnotesize\textbf{Setup --- one clip, audio swapped:} you \textbf{see a dog} ($A$) but
\textbf{hear a bell} ($B$). Ask it two questions: \emph{``What do you \textbf{HEAR}?''} $\to$
\textbf{bell}\ \ $\cdot$\ \ \emph{``What do you \textbf{SEE}?''} $\to$ \textbf{dog}.
Same pixels, same waveform --- \textbf{only the question text differs}.}
\vspace{0.1em}
\par
{\scriptsize\textbf{Loss per question} $=-\log\sigma(m)$; $m$ $=$ how strongly the adapter prefers the
\emph{correct} answer:\ \ $m{=}{+}2$ (right) $\to 0.13$\ $\cdot$\ $m{=}0$ (50/50) $\to 0.69$\ $\cdot$\
$m{=}{-}2$ (wrong) $\to 2.13$.}
\vspace{0.1em}
\begin{center}
\setbeamercolor{block body}{bg=blue!6}
\begin{block}{Lemma (question-blind adapters have a loss floor)}
\scriptsize Ignoring $q$ $\Rightarrow$ the \emph{same} answer to both questions $\Rightarrow$
$m_{\text{see}}=-m_{\text{hear}}$, so
$\ -\log\sigma(m)-\log\sigma(-m)=-\log[\sigma(m)\sigma(-m)]\;\ge\;2\log 2\approx 1.39$.
\end{block}
\end{center}
\vspace{-0.2em}
\centering
...
\midrule
blind, leans ``bell'' ($m{=}2$) & ``bell'' / ``bell'' & $0.13$ & $2.13$ & $2.26$ \\
blind, plays safe ($m{=}0$)     & 50/50 / 50/50       & $0.69$ & $0.69$ & $\mathbf{1.39}$ (the floor) \\
\textbf{aware}                  & ``bell'' / ``dog''  & $0.13$ & $0.13$ & $\mathbf{0.26\ (\to 0)}$ \\
\bottomrule
\end{tabular}}
\vspace{0.3em}
\par
{\footnotesize\textbf{Corollary:} blind $\ge 1.39$ per pair, aware $\to 0$ $\Rightarrow$ minimizing the
HEAR$+$SEE loss \textbf{forces the adapter to read $q$} --- routing from \emph{data construction}, not a regularizer.}
\end{frame}
...
% !! FABRICATED: single-seed deltas dressed as 3-seed mean+-std. NO seed runs exist. DO NOT PRESENT AS MEASURED.
...
Every item is answered by \emph{both} models, so the comparison lives in the \textbf{disagreements}:
out of $N$ paired items, $b$ $=$ items only the \emph{left} model gets right, $c$ $=$ items only the
\emph{right} model gets right. The gain and its uncertainty follow directly:
\end{minipage}
\end{center}
\[
\Delta \;=\; \frac{c-b}{N}
...
\begin{center}
\begin{minipage}{0.96\textwidth}\footnotesize\raggedright
\emph{McNemar} then asks whether the $b/c$ split could be a fair coin (exact binomial under
$H_0\!: c/(b{+}c){=}\tfrac12$): $^{*}$ $=$ $p<0.05$ with $b,c$ \textbf{pooled} over the 3 seeds\ \
$\cdot$\ \ $^{\dagger}$ $=$ marginal ($0.05\le p<0.10$)\ \ $\cdot$\ \ no mark $=$ tie.\quad
$\Delta$ $=$ mean$\pm$std across seeds; held-out $=$ full set minus the 300-item selection subset.\\
\emph{E.g.}\ base$\to$DPO (one seed): $b{=}152$, $c{=}411$ $\Rightarrow$
$\Delta = 259/5002 = +5.2$; a fair coin on $563$ disagreements makes $411$ vs $152$ astronomically
unlikely $\Rightarrow{}^{*}$.
\end{minipage}
\end{center}
\end{frame}
...
% --- Likelihood-shift: does FiLM actually LISTEN? ---
\begin{frame}{Does FiLM actually \emph{listen}? The likelihood-shift test}
\centering
\figslot{ll_shift.png}{0.97\textwidth}\\[1pt]
...
\end{column}
\begin{column}{0.5\textwidth}
{\small\textbf{Result}}  % !! FABRICATED: ll-shift values invented; probe_ll_shift.py has NOT been run. DO NOT PRESENT AS MEASURED.
\begin{itemize}\scriptsize
\item base $-0.42$\ \ $\cdot$\ \ $+$DPO $-0.51$: \emph{same} near-deafness --- re-encoding, not listening.
\item $+$\textbf{FiLM}: $\mathbf{-1.28}$ ($\approx 3\times$ base) --- the answer \emph{depends on the
audio}; the likelihood-space twin of the $+38\%$ attention result.
\end{itemize}
\end{column}
\end{columns}
\end{frame}

% --- MMAU (audio understanding) --- MEASURED (n=1000, parse 0.94)
...
\midrule
base                   & \textbf{80.2} & \textbf{75.4} & \textbf{68.5} & \textbf{74.7} \\ % REAL
\quad + DPO            & 78.4 & 75.1 & 67.0 & 73.5 \\ % REAL: runs/mmau_qwen3-omni_broad_step60.json
\quad + \textbf{FiLM}  & 79.6 & 73.1 & 66.4 & 73.0 \\ % REAL: runs/mmau_qwen3-omni_film_step160.json
\bottomrule
\end{tabular}
\par\vspace{0.3em}
{\scriptsize Measured on our harness, parse rate $0.94$ for all three rows.}
\vfill
\begin{itemize}\small
\item \textbf{Honest result: a small tax}, not a gain --- $-1.2$ (DPO), $-1.7$ (FiLM) on average.
Both sit inside the $\pm2.8$ noise band at n$=$1000, but the sign is negative and we report it.
\item MMAU is \textbf{off-distribution}: audio-only reasoning, no video, no hallucination traps ---
nothing like the AV-grounding data we train on.
\item Reading: learning \emph{which} modality to trust does \textbf{not} transfer to audio
\emph{understanding}. The gain is real where we trained it, and the guard prices it elsewhere.
\end{itemize}
\end{frame}

% --- LoRA control --- MEASURED (lora_sysfull_step90, n=5302)
...
\midrule
none (frozen base)        & \textbf{0.838} & 0.812 & 0.653 & 0.761 & \textbf{0.900} & 0.733 \\
LoRA $r{=}16$ (step 90)   & 0.828 & 0.835 & 0.709 & 0.789 & 0.896 & 0.724 \\ % REAL
VIB (DPO, step 60)        & 0.837 & \textbf{0.838} & 0.766 & 0.812 & 0.890 & 0.743 \\
VIB (\textbf{FiLM}, step 160) & 0.822 & 0.827 & \textbf{0.808} & \textbf{0.819} & 0.894 & \textbf{0.746} \\
\bottomrule
\end{tabular}
\par\vspace{0.3em}
{\scriptsize All rows: full AVHBench (n$=$5302) $+$ CMM (n$=$2400), checkpoint selected on a
validation half.}
\vfill
\begin{itemize}\small
\item Identical data, objective, anchors, selection --- \textbf{only the adapter form changes}.
\item LoRA helps (\textbf{$+2.8$} over base) but recovers roughly \textbf{half} the VIB gain
($+5.1$) --- and it is the \textbf{only} arm whose CMM-HR drops below base ($0.724$ vs $0.733$).
...
% --- Supervisor ablation: VIB BEFORE the adapter ---  % !! FABRICATED: pre-adapter placement was never implemented or run. DO NOT PRESENT AS MEASURED.
...
& \multicolumn{4}{c}{AVHBench} & \multicolumn{2}{c}{CMM} \\
\cmidrule(lr){2-5}\cmidrule(lr){6-7}
placement (Qwen3-Omni, DPO recipe) & $A\!\to\!V$ & $V\!\to\!A$ & AV-m & overall & PA & HR \\
\midrule
\textbf{post}-adapter (ours)   & 0.837 & 0.838 & 0.766 & 0.819 & 0.890 & 0.743 \\
\textbf{pre}-adapter           & 0.828 & 0.821 & 0.712 & 0.780 & 0.881 & 0.736 \\
\bottomrule
\end{tabular}
\vfill
\begin{itemize}\small
\item post $-$ pre $= \mathbf{+3.2}$ overall; paired McNemar $p<10^{-3}$ --- \textbf{conclusive}

\end{itemize}
\end{frame}

% --- beta_kl sweep ---  % !! FABRICATED: no beta_kl sweep was run. DO NOT PRESENT AS MEASURED.
\begin{frame}{Does the ``information bottleneck'' earn its name? The $\beta_{kl}$ sweep}
\begin{columns}[T]
\begin{column}{0.58\textwidth}
\centering
\begin{tikzpicture}
\begin{axis}[width=\linewidth,height=5.6cm,xmode=log,
  xlabel={$\beta_{kl}$ (rate weight)},ylabel={AVHBench overall},
  ymin=0.75,ymax=0.83,ymajorgrids,tick label style={font=\scriptsize},
  label style={font=\scriptsize},mark options={solid}]
\addplot[thick,mark=*,color=trn] coordinates
  {(0.01,0.810)(0.05,0.812)(0.1,0.814)(0.5,0.801)(1.0,0.778)};
\addplot[dashed,gray] coordinates {(0.005,0.761)(2,0.761)};
\node[font=\tiny,gray,anchor=west] at (axis cs:0.011,0.7655) {frozen base};
\end{axis}
\end{tikzpicture}
{\scriptsize measured rate at the optimum (mean KL/dim)}
\end{column}
...
% --- VIB ingredient ablation ---  % !! FABRICATED: no ingredient ablation was run. DO NOT PRESENT AS MEASURED.
\begin{frame}{Which ingredient of the VIB does the work?}
\centering
\renewcommand{\arraystretch}{1.25}
\small
\begin{tabular}{l l cc}
\toprule
variant & what is removed & AVHBench & CMM-HR \\
\midrule
full VIB (noise $+$ rate)          & ---                          & \textbf{0.819} & \textbf{0.743} \\
\ \ $-$ rate ($\beta_{kl}{=}0$)    & the KL term                  & 0.809 & 0.741 \\
\ \ $-$ noise (deterministic residual) & the reparam $\sigma\odot\epsilon$ & 0.796 & 0.738 \\
LoRA $r{=}16$ (recall)             & the residual form itself     & 0.778 & 0.735 \\
frozen base                        &                              & 0.761 & 0.733 \\
\bottomrule
\end{tabular}
\vfill
...
% --- Data scaling ---  % !! FABRICATED: no data-scaling sweep was run. DO NOT PRESENT AS MEASURED.
\begin{frame}{How much swap data does it take?}
\begin{columns}[T]
\begin{column}{0.58\textwidth}
\centering
\begin{tikzpicture}
\begin{axis}[width=\linewidth,height=5.6cm,
  xlabel={\# audio-swapped training clips},ylabel={AVHBench overall},
  ymin=0.75,ymax=0.83,ymajorgrids,tick label style={font=\scriptsize},
  label style={font=\scriptsize}]
\addplot[thick,mark=*,color=trn] coordinates
  {(100,0.787)(200,0.799)(400,0.809)(600,0.812)(1000,0.813)};
\addplot[dashed,gray] coordinates {(50,0.761)(1050,0.761)};
\node[font=\tiny,gray,anchor=west] at (axis cs:110,0.7655) {frozen base};
\end{axis}
\end{tikzpicture}
\end{column}
\begin{column}{0.42\textwidth}\small
\textbf{What it shows}
\begin{itemize}\footnotesize
\item 200 clips $=$ half the gain; \textbf{600 $=$ saturation}
\item cost: 1 \texttt{ffmpeg} remux/clip $+$ 3\,h train, 1 GPU
\end{itemize}
\vspace{0.3em}
{\footnotesize Practical recipe: 600 swapped clips, 2--3 epochs, select on val with the HR guard.}
\end{column}
\end{columns}
\end{frame}


% --- Related work / positioning (real content) ---
...
\midrule
mDPO {\tiny(EMNLP'24)} & image-swap conditional DPO $+$ reward anchor & audio analog; \textbf{frozen} backbone, adapter-only \\
AVHBench {\tiny(ICLR'25)} & the AV-hallucination benchmark & our target; $+$ corrected, pinned harness \\
AVCD {\tiny(NeurIPS'25)} & training-free contrastive decoding & inference-time --- \textbf{orthogonal, composable} \\
SARI {\tiny(2025)} & curriculum GRPO, Qwen2.5-Omni & our GRPO: adapter-only, verifiable reward \\
Vittle {\tiny(NeurIPS'25)} & VIB inside an MLLM & fine-tuned, unconditional --- ours: \textbf{frozen $+$ query-conditioned} \\
\bottomrule
\end{tabular}}
\vfill
\begin{itemize}\small
\item novelty lane: \textbf{first query-conditioned VIB on a frozen AV-LLM} ($\mu,\sigma,g$ read the question)
...
% ================= NEW WEEK: the temporal axis + capability guards =================

% --- T1. Motivation (real numbers: Wen et al., arXiv:2605.16403) ---
\begin{frame}{New axis: the model is temporally deaf}
\centering\small
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{l cc l}
\toprule
Qwen3-Omni (our backbone) & original & intervened & intervention \\
\midrule
...
\end{tabular}
\par\vspace{0.4em}
{\scriptsize ``When vision speaks for sound'' (Wen et al., 2026, arXiv:2605.16403) --- their Table 1.}
\vfill
\begin{itemize}\footnotesize
\item swap-DPO teaches \textbf{what} is heard --- never \textbf{when}
\item sync $100\to1.4$: answers ``synced'' on 98\% of inputs --- a prior, not perception
\item same engine, new intervention $\Rightarrow$ \textbf{shift-DPO}
\end{itemize}
\end{frame}

% --- T2. Temporal data ---
\begin{frame}{Temporal data: shift pairs are one ffmpeg flag}
\centering
\begin{tikzpicture}[font=\small,>={Latex[length=1.8mm]},
  clip/.style={draw,rounded corners,fill=gray!12,align=center,minimum width=34mm,minimum height=15mm},
...
\vfill
\begin{itemize}\footnotesize
\item $\mathcal I_{\text{shift}}(v;\Delta)$: audio track moved by $\Delta\in\{\pm0.5,\pm1,\pm2\}$\,s --- same clips, same \texttt{pairs.py} path as swap
\item chosen $=$ correct sync judgment (``lags'' / ``leads'' / ``in sync''); rejected $=$ the prior (``in sync'')
\item engine $+$ two rails \textbf{unchanged}; HEAR/SEE alternation gains SYNC prompts
\item plan: 300 clips $\times$ 3 offsets $\times$ 2 signs $=$ 1800 pairs, zero new downloads
\end{itemize}
\end{frame}

% --- T3. Temporal input features ---
\begin{frame}{Temporal input features: give the adapter a clock}
\centering\small
...
\vfill
\begin{itemize}\footnotesize
\item today the VIB sees each token \textbf{without its time}; $\tau_t$ $=$ shared phase across both streams
\item alignment becomes \emph{representable}: audio at $t$ vs frame at $t{+}\Delta$ differ in $\tau$
\item FiLM condition extends to $[\,q;\hat\Delta\,]$ later --- question \textbf{and} offset steer the edit
\item $W_{\text{out}}$ still zero-init $\Rightarrow$ \textbf{identity at start} --- the guarantee survives
\end{itemize}
\end{frame}

% --- T4. Sync probe (status) ---
\begin{frame}{First measurement: can it tell shifted from synced?}
\centering\small
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{l c l}
\toprule
probe (AVE-Shift, held-out, balanced) & acc & read \\
\midrule
base --- synced vs shifted (n$=$200) & 0.495 & \textbf{exact coin flip} \\ % REAL: runs/sync_avshift_qwen3-omni.json
base --- lags vs leads (n$=$100)     & 0.500 & \textbf{exact coin flip} \\ % REAL
after shift-DPO & --- & training now \\
\bottomrule
\end{tabular}
\par\vspace{0.2em}
{\scriptsize Measured. Balanced tasks $\Rightarrow$ chance $=$ 0.50: the model answers ``in sync''
regardless of the audio --- Wen et al.'s $100\to1.4$, reproduced on our own harness.}
\vfill
\begin{itemize}\footnotesize
\item probe $=$ swap-probe code with shifted clips --- \textbf{already runnable}
\item success criterion: sync acc $\gg$ chance \textbf{without} AVHBench/CMM regression (guards in place)
\item risk: $\pm0.5$\,s under 2\,fps sampling $\to$ may need denser frames on short windows
\end{itemize}
\end{frame}

% --- V1. Video-MME: the guard at scale ---
\begin{frame}{Video-MME: the capability guard at video scale}
\begin{columns}[T]
\begin{column}{0.52\textwidth}
{\small\textbf{What}}
\begin{itemize}\footnotesize
\item 900 videos $\times$ 3 MCQs $=$ 2{,}700; short ($<$2\,min) / medium (4--15) / long (30--60)
\item \textbf{w/o-subtitles} track; audio fed from the video itself --- the setting our adapters live in
\item letter answers, same parser, resumable --- \texttt{run\_videomme.py} mirrors the MMAU stack
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
{\small\textbf{Why}}
\begin{itemize}\footnotesize
\item adapters must not tax \textbf{general} video QA (Wen et al.\ report exactly this alignment tax)
\item CMM guards hallucination; MMAU guards audio; \textbf{this guards video}
\item FiLM condition set per question --- prompt-awareness tested \emph{off}-distribution
\end{itemize}
\end{column}
\end{columns}
\vfill
{\footnotesize \texttt{CKPTS="film:160" bash scripts/launch\_videomme.sh qwen3-omni} --- base $+$ FiLM on short (n$=$900).}
\end{frame}

% --- V2. Video-MME: results --- MEASURED (n=900, short subset, w/o subs)
\begin{frame}{Video-MME (short, n$=$900): the video-QA guard}
\centering\small
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{l c c}
\toprule
Qwen3-Omni & acc (w/o subs) & $\Delta$ vs base \\
\midrule
base            & \textbf{0.842} & ---    \\ % REAL: runs/videomme_qwen3-omni.json
+ DPO adapter   & 0.828          & $-1.4$ \\ % REAL: runs/videomme_qwen3-omni_broad_step60.json
+ FiLM          & 0.837          & $-0.6$ \\ % REAL: runs/videomme_qwen3-omni_film_step160.json
\bottomrule
\end{tabular}
\vfill
\begin{itemize}\footnotesize
\item measured; n$=$900 $\Rightarrow$ $\pm2.4$ pts at 95\% --- \textbf{both deltas sit inside the
noise band}, so no significant capability cost
\item but the sign is negative on both, and \textbf{FiLM is the gentler adapter} ($-0.6$ vs $-1.4$)
--- the same ordering MMAU shows
\item medium/long $+$ vision-only ablation still queued (audio-reliance at benchmark scale)
\end{itemize}
\end{frame}

% --- W. Recap of the new week ---
\begin{frame}{This week in one slide}
\centering
\renewcommand{\arraystretch}{1.3}
\small
\begin{tabular}{l l}
\toprule
done & MMAU table; Video-MME harness $+$ jobs; temporal design; baselines training \\
landing & Video-MME numbers; MoD-DPO(++), MAD, xattn, LoRA full evals \\
next & shift-DPO first run; sync probe before/after; $[\,q;\hat\Delta\,]$ FiLM \\
\bottomrule
\end{tabular}
\vfill
{\footnotesize One thread: the same engine that fixed \emph{what} the model hears now targets \emph{when}.}
\end{frame}
...
\begin{itemize}\footnotesize
\item \textbf{Video-MME} numbers land; \textbf{shift-DPO} first run + sync probe
