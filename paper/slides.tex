...
{\small\textbf{Step 1 --- encode the question into one vector.}}
\vspace{-0.4em}
\[
q \;=\; W_q\,\tfrac1L\textstyle\sum_{t} E(w_t)
\]
\vspace{-1.2em}
\begin{center}
{\scriptsize $w_{1:L}$ = the question's tokens;\ $E$ = the LLM's own \emph{frozen} embedding table;\
$W_q$ = a small trained projection.}
\end{center}
{\small\textbf{Step 2 --- let $q$ modulate the hidden activations} (FiLM: per-feature scale $+$ shift).}
\vspace{-0.4em}
\[
[\gamma;\beta]=W_{\!f}\,q \qquad\quad \tilde h=(1+\gamma)\odot h+\beta
\]
\vspace{-1.2em}
\begin{center}
{\scriptsize so the code's heads now read the question: $\mu(x)\to\mu(x,q)$,\ \ $\sigma(x)\to\sigma(x,q)$.}
\end{center}
{\small\textbf{Step 3 --- let $q$ gate the edit} (one scalar per modality).}
\vspace{-0.4em}
\[
g(q)=\mathrm{sigm}(w_g^{\!\top}q+b_g)\in(0,1) \qquad\quad
\boxed{\;y \;=\; x \;+\; g(q)\,W_{\text{out}}\,z\;}
\]
\vspace{-0.6em}
{\footnotesize\textbf{Identity at init, for every $q$}: $W_{\text{out}}{=}0,\ W_{\!f}{=}0$
$\Rightarrow y\equiv x$ (the residual is $g\cdot 0\cdot z=0$) $\Rightarrow$ \texttt{bypass} still
returns the \textbf{exact frozen base} --- the DPO reference stays free.\quad
New trainables: $W_{\!f},w_g$ per modality $+$ one shared $W_q$.}
\end{frame}
...
\]
\vspace{-0.9em}
{\footnotesize Same KL term, read conditionally (the per-$q$ bound holds with the $q$-independent prior;
average over $q$). The code may now \textbf{keep the bits that answer \emph{this} question} --- audio bits
for an audio question --- instead of one budget for all questions.\par
At $\beta_{kl}{=}0.05$ the rate binds weakly: the \emph{design principle}; the conditioning does the
empirical work (hence the $\beta_{kl}$ sweep next week).}
\end{frame}
...
\footnotesize For any adapter that ignores $q$, the two swap-DPO margins on $x^\star$ are exact negatives,
...
\]
\vspace{-1.1em}\par
with equality only at $m=0$ (learn \emph{nothing}). A question-\emph{aware} adapter can drive both losses $\to 0$.
\end{block}
\end{center}
{\small\textbf{Corollary.} Any minimizer of the HEAR$+$SEE objective must condition on $q$:
\textbf{routing is forced by data construction} (gap $=2\log 2$), not encouraged by a regularizer.}
\vspace{0.3em}
{\footnotesize The minimizer we actually find (from the weights): conditioning lands in the \emph{vision}
FiLM ($\lVert W_{\!f}^{(v)}\rVert\!\approx\!4.5\,\lVert W_{\!f}^{(a)}\rVert$); the gate stays $\approx$\,open ---
question-\emph{modulated} re-encoding rather than hard routing.}
\end{frame}
...
One panel per variant (base / $+$FiLM). \textbf{[TBD: probe\_ll\_shift $\to$ plot\_ll\_shift]}}
\end{column}
