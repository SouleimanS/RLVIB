...
\textbf{pre}-adapter           & 0.834 & 0.835 & 0.760 & 0.809 & 0.887 & 0.740 \\
\bottomrule
\end{tabular}
\vfill
\begin{itemize}\small
\item \textbf{Placement barely matters}: $-0.3$ overall, paired McNemar $p=0.41$ --- a tie on every axis.
\item Reading: the adapter is near-linear, so the residual edit approximately \emph{commutes} with it ---
what matters is editing the modality stream \textbf{before fusion}, not the exact insertion point.
\item We keep \textbf{post}: one attach dim (the LLM width) for both modalities, and the code stays
...
\textbf{What it shows}
\begin{itemize}\footnotesize
\item \textbf{Plateau} for $\beta_{kl}\!\in\![0.01,0.1]$: the rate barely binds --- compression is
\emph{not} the mechanism there.
\item \textbf{Cliff} at $\beta_{kl}\!\ge\!0.5$: the code is squeezed hard (rate $\uparrow$ 100$\times$)
and grounding degrades --- over-compression destroys the signal.
\item Mild optimum at $0.1$: a useful \emph{regularizer}, honestly not the driver.
\end{itemize}
\vspace{0.3em}
{\footnotesize So ``IB'' earns its name only as a guard-rail --- the framing we now use in the write-up.}
\end{column}
\end{columns}
\end{frame}
...
\vfill
\begin{itemize}\small
\item \textbf{The reparametrization noise is the active ingredient} ($+1.6$ of the $+5.1$ vanishes
without it): stochastic answers smooth the preference landscape during training --- the same reason
the GRPO variant gets its exploration for free.
\item The \textbf{rate term is minor} ($+0.3$) --- consistent with the $\beta_{kl}$ sweep: a guard-rail.
\item Echoes the LoRA control: it is the \textbf{stochastic zero-init residual}, not parameter count.
\end{itemize}
\end{frame}
...
\textbf{What it shows}
\begin{itemize}\footnotesize
\item Half the gain arrives by \textbf{200 clips}; saturation by $\sim$\textbf{600}.
\item A swapped clip costs one \texttt{ffmpeg} remux --- the whole training set is
$\sim$40 GPU-minutes of preprocessing $+$ a 3-hour train.
\item The grounding signal is \textbf{cheap}: the bottleneck is the \emph{objective}, not the data.
\end{itemize}
\vspace{0.3em}
