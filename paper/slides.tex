...
{\footnotesize\textbf{Setup --- one single clip.} The audio is swapped: you \textbf{see a dog} ($A$)
but \textbf{hear a bell} ($B$). Ask the \emph{same clip} two questions:\\
\centerline{``What do you \textbf{HEAR}?'' $\to$ correct: \textbf{bell} \qquad\qquad
``What do you \textbf{SEE}?'' $\to$ correct: \textbf{dog}}
The correct answer \textbf{flips} --- but pixels and waveform are \emph{identical};
\textbf{only the question text differs}.}
\vspace{0.1em}
\par
{\scriptsize\textbf{Loss per question} $=-\log\sigma(m)$, where the margin $m$ $=$ how strongly the
adapter prefers the \emph{correct} answer:\quad
$m{=}{+}2$ (right) $\to$ loss $0.13$\ \ $\cdot$\ \ $m{=}0$ (50/50) $\to$ $0.69$\ \ $\cdot$\ \
$m{=}{-}2$ (wrong) $\to$ $2.13$.}
\vspace{0.1em}
\begin{center}
\setbeamercolor{block body}{bg=blue!6}
\begin{block}{Lemma (question-blind adapters have a loss floor)}
\scriptsize Ignoring $q$ $\Rightarrow$ the \emph{same} answer to both questions $\Rightarrow$ the two
margins are exact negatives, $m_{\text{see}}=-m_{\text{hear}}$, so
\vspace{-0.3em}
\[
...
\toprule
adapter & answers & HEAR loss & SEE loss & total \\
\midrule
blind, leans ``bell'' ($m{=}2$) & ``bell'' / ``bell'' & $0.13$ & $2.13$ & $2.26$ \\
blind, plays safe ($m{=}0$)     & 50/50 / 50/50       & $0.69$ & $0.69$ & $\mathbf{1.39}$ (the floor) \\
\textbf{aware} ($m_{\text{hear}}{=}m_{\text{see}}{=}2$) & ``bell'' / ``dog'' & $0.13$ & $0.13$ & $\mathbf{0.26\ (\to 0)}$ \\
\bottomrule
\end{tabular}}
\vspace{0.5em}
\par
{\footnotesize\textbf{Corollary.} Blind adapters are stuck $\ge 1.39$ per pair; aware ones reach $0$ ---
so minimizing the HEAR$+$SEE loss \textbf{forces the adapter to read $q$}: routing comes from the
\emph{data construction}, not a regularizer.}
\end{frame}
