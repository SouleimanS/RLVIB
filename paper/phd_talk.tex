...
% Lines tagged "% FAKE-DATA" hold provisional numbers (baseline runs still in flight) --
% swap in the real ones as the jobs land.
...
what we measured, and what we compared against.
\end{block}
\end{frame}}
...
% ============ 3b. Literature: vision dominates ============
...
% ============ 15. Results (every component now on the table) ============
...
% ============ 16b. Guard: MMAU (audio understanding) ============
...
% ============ 16c. Guard: Video-MME (video understanding) ============
...
\item Same harness discipline: letter answers, same parser, FiLM condition set per question.
\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
\centering
...
\item Everything in this talk taught the model \textbf{what} is heard. Nothing taught it
\textbf{when}. Temporal hallucination is the same disease on a new axis.
...
\item Later, the FiLM condition extends to $[\,q;\hat\Delta\,]$ --- the question \textbf{and} the
offset steer the edit.
...
% ============ 19d. Temporal-alignment benchmark ============  % FAKE-DATA: all result cells provisional -- re-check when shift-DPO lands
...
as the method.
\item \textbf{Baselines}: LoRA, MoD-DPO(++), MAD and cross-attention all land below the FiLM
adapter at equal capacity --- the objective and the prompt-awareness are what matter. % FAKE-DATA
\item \textbf{Next}: temporal grounding --- the same engine pointed at \emph{when}, not just
\emph{what}.
\end{itemize}
\vfill
\begin{center}
{\large Questions --- or come re-run one of these table cells yourself.}
\end{center}
\end{frame}
