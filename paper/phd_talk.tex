...
\begin{frame}{Why it collapses: likelihood displacement}
\begin{itemize}\small
\item The loss watches only the \textbf{margin}, so it also falls when the rejected answer sinks
\emph{faster} than the chosen one --- \textbf{both can sink together}.
\item The freed probability mass must land somewhere: it drains onto a third answer that is always
``safe'' on hallucination data --- the constant \textbf{``No''}.
\end{itemize}
\vspace{0.3em}
\centering
\begin{tikzpicture}[font=\tiny]
  ...
  \node at (3.5mm,-2.5mm)  {chosen};
  \node at (13.5mm,-2.5mm) {rejected};
  ...
  \end{scope}
\end{tikzpicture}\\[2pt]
{\scriptsize Cartoon numbers. The margin (chosen $-$ rejected, in log-prob) \emph{grew}, so the
\textbf{loss is satisfied} --- while both real answers died. Our actual run: PA $0.95\to0.10$,
every answer ``No''. Known as \emph{likelihood displacement} (Razin et al., 2024).}
\vfill
\begin{center}
\setbeamercolor{block body}{bg=green!8}
\begin{block}{}
\centering\small The cure cannot be another metric --- the \textbf{loss itself} must pin the chosen
answer's own probability. That is exactly rail 1, next slide.
\end{block}
\end{center}
\end{frame}
