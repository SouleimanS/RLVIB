...
\institute{Research internship --- final presentation\\[2pt]
  \small Host lab: AIST CVRT \quad$\cdot$\quad Supervisor: Qiu Yue}
\date{\today}
...
\vspace{-1.2em}
\begin{block}{}
\small Audio-visual LLMs frequently answer questions about sound using visual evidence. This work
...
\end{tabular}
\vfill
\small The model over-weights visual evidence relative to auditory evidence: it reports objects
...
\centering\small A model that answers ``no'' to every question attains HR $=$ 1.0 and PA $=$ 0.
...
\begin{block}{}
\centering\small Reported accuracies are meaningful only relative to a fixed evaluation protocol.
\end{block}
\end{center}
\end{frame}
...
{\small The information bottleneck (Tishby et al., 2000) seeks a compressed representation $Z$ of
...
\vfill
\small Substituting a clip's audio track produces an instance in which the visible and audible
...
\begin{block}{}
\centering\small A hallucination metric alone is not a sufficient training signal.
\end{block}
\end{center}
\end{frame}
...
{\small Consider one substituted clip (visible: dog; audible: bell) and two questions over
...
\vspace{0.5em}
\par
{\small A query-independent adapter has a loss floor of $2\log 2 \approx 1.39$ per pair, whereas a
...
mechanisms, only one of which corresponds to the hypothesized account.}
\end{column}
\end{columns}
\end{frame}

\begin{frame}{Mechanism II: analysis of the learned parameters}
{\small Three measurements computed directly from the trained FiLM checkpoint, without inference:}
\vspace{0.3em}
\centering
...
\end{tabular}
\par\vspace{0.3em}
{\scriptsize Measured. The model reports synchronization independently of the audio content.}
\vfill
\begin{itemize}\small
\item The training signal used so far concerns \emph{which} event is heard, not \emph{when} it
occurs.
...
\item[] {\footnotesize\emph{Status: implemented; training in progress. No trained results are
reported here.}}
\end{itemize}
\end{frame}
...
\end{tabular}
\vfill
{\small Several of these results informed the final design and are reported for completeness.}
\end{frame}
...
\setbeamercolor{block body}{bg=blue!6}
\begin{block}{}
\centering\small Every value reported here is either measured under our protocol (with the source
...
\setbeamercolor{block body}{bg=blue!6}
\begin{block}{}
\centering\small A parameter-efficient adapter improves audio grounding in a frozen audio-visual
