...
\end{tabular}
\par\vspace{0.3em}
{\scriptsize Measured. Differences from baseline are within the $\pm7$-point interval at these
sample sizes. The trained variant used the substituted-timing data \emph{without} temporal input
features.}
\vfill
\begin{itemize}\footnotesize
...
\item \textbf{This is not sufficient.} Training on the displaced data alone leaves accuracy at
chance. We attribute this to representational capacity rather than to the objective: the bottleneck
receives each token without positional information, so temporal misalignment is not expressible in
its input (see next slide).
\end{itemize}
\end{frame}
...
\item[] {\footnotesize\emph{Status: implemented but not yet trained. This yields a falsifiable
prediction: the same displaced-audio data combined with temporal features should move accuracy off
chance, whereas the data alone did not.}}
\end{itemize}
\end{frame}
